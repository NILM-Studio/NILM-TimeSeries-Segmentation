...
from elec_feature_analyze.time_clustering.cluster_result_analyze import cluster_result_quantification
...
def gpu_dtw_matrix_torch(ts_list, device="cuda:0"):
    ...
    if not TORCH_AVAILABLE:
        print("❌ PyTorch不可用，无法使用GPU加速DTW，将使用CPU替代方案")
        print("提示: 如果遇到 WinError 1114，通常是由于 PyTorch 环境问题，建议检查或重装 torch解决anaconda的data虚拟环境的报错")
        sys.stdout.flush()
        # 返回None或调用其他CPU实现
        return fast_dtw_matrix_tslearn(ts_list, "temp")
...
if __name__ == "__main__":
    ...
    # 数据源信息
    data_path = os.path.join(BASE_DIR, 'detsec_features.npy')
    seq_len_path = os.path.join(BASE_DIR, 'data', 'seq_length.npy')
    print(f"\n【数据源配置】")
    ...
    for i in range(len(data)):
        normalized_ts_list.append(data[i])
        # if seq_len[i] == 0:
        #     continue
    ...
    path_parts = norm_base_dir.split(os.sep)
    appliance_name = path_parts[-1] if path_parts[-1] else (path_parts[-2] if len(path_parts) > 1 else "unknown")
    
    result_save_path = os.path.join(BASE_DIR, f'result/{eps}_{min_pts}/detsec_clust_assignment.npy')

    # 确保输出路径存在
    os.makedirs(os.path.dirname(result_save_path), exist_ok=True)

    # 获取聚类评估指标
    sil_score, db_score, ch_score = cluster_result_quantification(labels, dist_matrix, data, None)
    ...
    # 保存JSON文件
    json_save_path = os.path.join(BASE_DIR, f"result/{eps}_{min_pts}/evaluation_metrics.json")
